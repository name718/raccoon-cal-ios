...
struct HomeView: View {
    ...
    private func randomEncouragement() -> String {
        ...
        switch petMood {
        case .happy:
            phrases = ["太棒了！今天的饮食很均衡。", "你做到了！继续保持。", "完美！今天状态非常好。"]
        case .satisfied:
            phrases = ["不错哦，继续记录吧。", "已经记录了好几餐，加油！", "状态很好，继续努力。"]
        case .normal:
            phrases = ["今天才刚开始，加油记录！", "别忘了记录午餐和晚餐。", "下一餐也记得继续记录。"]
        case .hungry:
            phrases = ["快去记录今天的第一餐吧！", "先去吃点东西，再来记录。", "开始记录，养成好习惯。"]
        case .sad:
            phrases = ["今天稍微超标了，明天继续加油。", "没关系，明天会更好。", "别灰心，你一定能做到。"]
        case .missing:
