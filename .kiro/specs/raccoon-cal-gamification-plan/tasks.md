...
- [x] 16.1 创建 `Views/Home/HomeView.swift`
- [x] 16.2 实现顶部信息栏（昵称/Streak 火焰图标/等级）
- [x] 16.3 集成 `CalorieRingView` 展示当日卡路里进度
- [x] 16.4 集成 `HPHeartView` 展示当日生命值
- [x] 16.5 实现三餐 + 加餐卡路里小计列表，点击跳转 RecordView
- [x] 16.6 集成 `RaccoonMoodView` 展示浣熊主场景（静态图片，根据心情切换）；点击浣熊调用 `interactWithPet` 并显示随机鼓励文案
- `[DEFERRED]` 浣熊互动 3D 动画（摇尾巴/眨眼/跳跃）
- [x] 16.7 实现每日任务进度区域（已完成数/总数），点击展开任务详情列表
- [x] 16.8 实现 `onAppear` 时拉取游戏化状态和今日饮食记录
- [x] 16.9 实现超标时浣熊切换难过状态和"今日已超标"提示
...
- [x] 17.1 创建 `Views/Camera/CameraView.swift`
- [x] 17.2 集成 `AVFoundation` 实现相机取景框实时预览
- [x] 17.3 实现拍照按钮，捕获图像并上传至 `/api/food/recognize`
- [x] 17.4 实现识别结果展示（食物名称/卡路里/蛋白质/脂肪/碳水）
- [x] 17.5 实现识别失败提示和手动输入食物名称的降级入口
- [x] 17.6 实现用户修改食物名称/份量/餐次的编辑表单
- [x] 17.7 实现确认保存，调用 `saveFoodRecord`，触发 XP 浮动动画
- [x] 17.8 实现从相册选取图片（`PHPickerViewController`）
- [x] 17.9 实现相机权限检查，未授权时展示引导弹窗
- [x] 17.10 实现多食物识别结果列表，支持单独勾选
- [x] 17.11 保存成功后刷新浣熊饱食度数据（静态图片更新）
- `[DEFERRED]` 浣熊进食 3D 动画
...
- [x] 18.1 创建 `Views/Record/RecordView.swift`
- [x] 18.2 实现日历视图（过去 30 天打卡状态，已打卡用主色标记）
- [x] 18.3 实现点击日期展示该日饮食记录列表（按餐次分组）
- [x] 18.4 实现长按记录弹出删除确认，删除后重新计算当日卡路里
- [x] 18.5 实现过去 7 天卡路里折线图（含目标虚线）
- [x] 18.6 实现过去 7 天三大营养素柱状图
- [x] 18.7 实现下拉刷新重新拉取数据
- [x] 18.8 实现无记录时的引导文案和跳转 CameraView 按钮
...
- [x] 20.1 完善 `Views/Profile/ProfileView.swift`（当前为占位符）
- [x] 20.2 实现用户信息展示（昵称/头像/身高/体重/年龄/目标/活动水平）
- [x] 20.3 实现个人信息编辑页，保存后触发卡路里目标重算
- [x] 20.4 实现健康数据摘要（累计记录天数/食物次数/平均卡路里）
- [x] 20.5 实现等级/XP/等级进度条展示
- [x] 20.6 实现成就徽章网格（已解锁/未解锁，含进度百分比）
- [x] 20.7 实现联盟信息（当前联盟/本周排名/Top 10 排行榜）
- [x] 20.8 实现体重历史折线图（最近 30 天）
- [x] 20.9 实现通知设置（打卡提醒/任务刷新/联盟结算，可修改时间）
- [x] 20.10 实现联盟结算结果弹窗（晋升/降级提示）
